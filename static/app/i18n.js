...
const translations = {
    'zh-CN': {
        ...
        'config.kiroIdentity.title': 'Kiro 身份注入',
        'config.kiroIdentity.providerTag': 'claude-kiro-oauth 专用',
        'config.kiroIdentity.intro': '默认关闭，请求会原样转发到 Kiro 上游，不做任何身份覆盖。开启后会在 system 提示词前注入一段自定义文案，用于"伪装"成其他客户端身份（如 Cline / Kilo Code / Claude Code），适合多场景接入。',
        'config.kiroIdentity.enabled': '启用身份注入前缀',
        'config.kiroIdentity.enabledNote': '关闭 = 直接透传 Kiro 原始响应（推荐）；开启 = 注入下方前缀进行身份伪装。',
        'config.kiroIdentity.fallbackName': '兜底身份名',
        'config.kiroIdentity.fallbackNameNote': '当用户的 system prompt 里没有指定身份时，模型会用这个名字回答自己是谁。留空 = 使用代码内置默认。',
        'config.kiroIdentity.prompt': '前缀模板',
        'config.kiroIdentity.promptNote': '支持占位符 {{fallbackName}} 和 {{identity}}。留空 = 使用代码内置默认模板。',
        'config.kiroIdentity.resetToDefault': '恢复默认模板',
    ...
    'en-US': {
        ...
        'config.kiroIdentity.title': 'Kiro Identity Injection',
        'config.kiroIdentity.providerTag': 'claude-kiro-oauth only',
        'config.kiroIdentity.intro': 'Disabled by default — requests are forwarded to Kiro upstream unmodified. When enabled, a custom prefix is injected at the top of the system prompt to "spoof" the assistant as another client identity (Cline / Kilo Code / Claude Code, etc.), useful for multi-scenario integration.',
        'config.kiroIdentity.enabled': 'Enable Identity Prefix Injection',
        'config.kiroIdentity.enabledNote': 'OFF = forward Kiro responses untouched (recommended). ON = inject the prefix below to spoof identity.',
        'config.kiroIdentity.fallbackName': 'Fallback Identity Name',
        'config.kiroIdentity.fallbackNameNote': 'Used when the user\'s system prompt does not specify an identity. Leave empty to use the built-in default.',
        'config.kiroIdentity.prompt': 'Prefix Template',
        'config.kiroIdentity.promptNote': 'Supports placeholders {{fallbackName}} and {{identity}}. Leave empty to use the built-in default template.',
        'config.kiroIdentity.resetToDefault': 'Restore Default Template',
